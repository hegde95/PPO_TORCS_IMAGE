...
class TorcsEnv:
    ...
    def make_observaton(self, raw_obs):
        if self.vision is False:
            ...
            r = np.flip(np.flip(image_rgb[:,0].reshape(64,64)),1)
            g = np.flip(np.flip(image_rgb[:,1].reshape(64,64)),1)
            b = np.flip(np.flip(image_rgb[:,2].reshape(64,64)),1)
            image_rgb = np.array([r,g,b])
            # image_rgb = np.array([r,g,b]).swapaxes(1,2).swapaxes(2,0)
